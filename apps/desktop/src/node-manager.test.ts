...
describe("nodeDownloadFallbackUrl", () => {
	it("returns undefined when no mirrorFallback is configured", () => {
		const config: NodeVersionsConfig = {
			version: "24.15.0",
			minSystemNode: 18,
			mirrorBase: "https://registry.npmmirror.com/-/binary/node",
		...
		expect(nodeDownloadFallbackUrl(config, "darwin", "arm64")).toBeUndefined();
	});
	it("returns undefined when mirrorFallback is empty string", () => {
		const config: NodeVersionsConfig = {
			version: "24.15.0",
			minSystemNode: 18,
			mirrorBase: "https://registry.npmmirror.com/-/binary/node",
	...
	it("builds the fallback URL from mirrorFallback", () => {
		const config: NodeVersionsConfig = {
			version: "24.15.0",
			minSystemNode: 18,
			mirrorBase: "https://registry.npmmirror.com/-/binary/node",
			mirrorFallback: "https://nodejs.org/dist",
			checksums: {},
		};
		expect(nodeDownloadFallbackUrl(config, "darwin", "arm64")).toBe(
			"https://nodejs.org/dist/v24.15.0/node-v24.15.0-darwin-arm64.tar.gz",
		);
	});
	it("builds the fallback URL for win32", () => {
		const config: NodeVersionsConfig = {
			version: "24.15.0",
			minSystemNode: 18,
			mirrorBase: "https://registry.npmmirror.com/-/binary/node",
		...
		expect(nodeDownloadFallbackUrl(config, "win32", "x64")).toBe(
