...
export interface PresetRuntimeInstaller {
  ...
  install(
    ...
    missing: readonly PresetRuntimeDependencyId[],
  ): Promise<void>;
}
...
function uniquePathEntries(entries: readonly (string | undefined)[]): string {
  return [
    ...new Set(
      entries.flatMap((value) => value?.split(delimiter) ?? []).filter(Boolean),
    ),
  ].join(delimiter);
}
...
export function withPresetRuntimeEnvironment(
  ...
  return {
    ...
    NODE_PATH: uniquePathEntries([
      join(paths.packages, "node_modules"),
...
function installedBinaryPath(
  ...
  if (typeof binary !== "string")
    throw new Error(`${packageName} did not expose its installed binary`);
  const absolute = resolve(binary);
  const modules = resolve(paths.packages, "node_modules");
  const rel = relative(modules, absolute);
  if (rel.startsWith("..") || rel === "")
    throw new Error(`${packageName} resolved outside the managed runtime`);
  if (!existsSync(absolute))
    throw new Error(`${packageName} binary is missing`);
  return absolute;
}

function availableInstalledBinary(
  paths: PresetRuntimePaths,
  packageName: string,
): string | undefined {
  try {
...
class DesktopPresetRuntimeInstaller implements PresetRuntimeInstaller {
  ...
  async prepare(): Promise<void> {
    ...
    const ffmpeg = availableInstalledBinary(
      this.paths,
    ...
    const ffprobe = availableInstalledBinary(
      this.paths,
  ...
  private async installVideo(
    missing: readonly PresetRuntimeDependencyId[],
  ): Promise<void> {
    if (
      missing.some(
        (id) => id === "hyperframes" || id === "ffmpeg" || id === "ffprobe",
      )
    ) {
  ...
  private async installReport(
    missing: readonly PresetRuntimeDependencyId[],
  ): Promise<void> {
    if (
      missing.some(
        (id) => id === "echarts" || id === "playwright" || id === "chromium",
      )
    ) {
      await this.installPackages([
    ...
    if (missing.includes("openpyxl")) {
      ...
      if (python === undefined)
        throw new Error("Python is required before openpyxl can be installed");
    ...
    if (missing.includes("chromium")) {
      ...
      if (!existsSync(cli))
        throw new Error("Playwright CLI is missing after package installation");
  ...
  private async resolvePython(): Promise<
    { command: string; prefix: readonly string[] } | undefined
  > {
...
function initialDependencies(
  presetId: ManagedPresetRuntimeId,
): PresetRuntimeDependency[] {
  const ids: readonly PresetRuntimeDependencyId[] =
    presetId === VIDEO_ID
...
export class PresetRuntimeController {
  ...
  constructor(private readonly options: PresetRuntimeControllerOptions) {
    ...
    this.processAdapter = options.processAdapter ?? nativeRuntimeProcess;
    this.installer =
      options.installer ?? new DesktopPresetRuntimeInstaller(options);
    this.env = managedEnvironment(options, this.paths);
  ...
  check(presetId: ManagedPresetRuntimeId): Promise<PresetRuntimeSnapshot> {
    ...
    if (
      this.activeInstall?.presetId === presetId &&
      this.snapshots.get(presetId)?.phase === "installing"
    ) {
      return Promise.resolve(
        this.snapshots.get(presetId) as PresetRuntimeSnapshot,
      );
    }
    this.publish(
      presetId,
      "checking",
      this.snapshots.get(presetId)?.dependencies ??
        initialDependencies(presetId),
    );
    const operation = this.inspect(presetId)
      .then((dependencies) => {
        return this.publish(
          presetId,
          dependencies.every((item) => item.state === "ready")
            ? "ready"
            : "missing",
          dependencies,
        );
  ...
  install(presetId: ManagedPresetRuntimeId): Promise<PresetRuntimeSnapshot> {
    if (this.activeInstall?.presetId === presetId)
      return this.activeInstall.promise;
    if (this.activeInstall !== undefined) {
      return this.activeInstall.promise.then(() => this.install(presetId));
    }
    const operation = this.check(presetId)
      .then(async (checked) => {
        if (checked.phase === "ready" || !checked.canInstall) return checked;
        const missing = checked.dependencies.filter(
          (item) => item.state !== "ready" && item.installable,
        );
        this.publish(
          presetId,
          "installing",
          checked.dependencies.map((item) =>
            missing.some((candidate) => candidate.id === item.id)
              ? { ...item, state: "installing" as const }
              : item,
          ),
        );
        try {
          ...
          const current =
            this.snapshots.get(presetId)?.dependencies ?? checked.dependencies;
          return this.publish(
            presetId,
            "failed",
            current.map((item) =>
      ...
      .finally(() => {
        if (this.activeInstall?.promise === operation)
  ...
  private async inspect(
    presetId: ManagedPresetRuntimeId,
  ): Promise<PresetRuntimeDependency[]> {
    try {
  ...
  private async inspectVideo(): Promise<PresetRuntimeDependency[]> {
    ...
    const hyperframes =
      readPackageVersion(this.paths, "hyperframes") === HYPERFRAMES_VERSION &&
      existsSync(hyperframesCli)
        ? HYPERFRAMES_VERSION
        : undefined;
    const ffmpeg = await this.probe(
      availableInstalledBinary(this.paths, "@ffmpeg-installer/ffmpeg") ??
        "ffmpeg",
      ["-version"],
    );
    const ffprobe = await this.probe(
      availableInstalledBinary(this.paths, "@ffprobe-installer/ffprobe") ??
        "ffprobe",
      ["-version"],
    );
    return [
  ...
  private async inspectReport(): Promise<PresetRuntimeDependency[]> {
    ...
    const openpyxl =
      python === undefined
        ? undefined
        : await this.probe(python.command, [
            ...python.prefix,
            "-c",
            "import openpyxl; print(openpyxl.__version__)",
          ]);
    const echarts =
      readPackageVersion(this.paths, "echarts") === ECHARTS_VERSION
        ? ECHARTS_VERSION
        : undefined;
    const playwright =
      readPackageVersion(this.paths, "playwright") === PLAYWRIGHT_VERSION
        ? PLAYWRIGHT_VERSION
        : undefined;
  ...
  > {
    for (const candidate of this.platform === "win32"
      ...
      const version = await this.probe(candidate.command, [
        ...candidate.prefix,
        "--version",
      ]);
  ...
  private async chromiumVersion(): Promise<string | undefined> {
    if (readPackageVersion(this.paths, "playwright") !== PLAYWRIGHT_VERSION)
      return undefined;
    const executable = await this.probe(this.options.nodeExecutable, [
      "-e",
  ...
  private publish(
    ...
    const snapshot: PresetRuntimeSnapshot = Object.freeze({
      ...
      dependencies: Object.freeze(
        dependencies.map((item) => Object.freeze({ ...item })),
      ),
      canInstall: dependencies.some(
        (item) => item.state !== "ready" && item.installable,
      ),
