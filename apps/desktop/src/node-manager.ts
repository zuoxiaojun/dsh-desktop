...
export function nodeDownloadFallbackUrl(
	config: NodeVersionsConfig,
	platform: NodeJS.Platform,
	arch: string,
): string | undefined {
	if (config.mirrorFallback === undefined || config.mirrorFallback === "")
		return undefined;
	const { fileName } = nodeArchiveSpec(config.version, platform, arch);
	return `${config.mirrorFallback}/v${config.version}/${fileName}`;
}
...
export async function installManagedNode(options: {
	...
	const primaryUrl = nodeDownloadUrl(config, platform, arch);
	const fallbackUrl = nodeDownloadFallbackUrl(config, platform, arch);
	...
	try {
		...
		for (const currentUrl of urls) {
			...
			for (;;) {
				...
				const isFallback = currentUrl !== primaryUrl;
				const label = isFallback
					? `备用 ${currentUrl}`
					: `Node.js v${config.version}`;
				onProgress?.({
					stage: "downloading",
					detail: label,
					percent: 0,
				...
				try {
					...
					if (attempt >= 3 || signal?.aborted) {
						if (currentUrl === urls[urls.length - 1]) throw error;
						break;
					}
					onProgress?.({
						stage: "downloading",
						detail: isFallback
							? `备用镜像下载失败，第 ${attempt} 次重试…`
							: `下载失败，第 ${attempt} 次重试…`,
					});
				}
			}
		...
		if (!(await hashMatches(archivePath, expected))) {
			throw new Error(`SHA256 mismatch for ${primaryUrl}`);
		}
...
function getDshLatestVersion(
	...
	return new Promise((resolve) => {
		let url: URL;
		try {
			url = new URL(dshRegistryLatestUrl(registry));
		} catch {
			resolve(undefined);
			return;
		}
...
export async function installDshUpdate(options: {
	...
	try {
		const latest = await getDshLatestVersion(MANAGED_REGISTRY);
		const installed = readManagedDshVersion(userDataDir);
		if (!shouldUpdateDsh(installed, latest) || signal?.aborted)
			return undefined;
