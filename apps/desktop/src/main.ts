...
import {
  reloadWithHeldFrame,
  type HeldReloadFrame,
} from "./window-reload-transition.ts";
...
function applyPluginRecoverySnapshot(
  snapshot: PluginRecoverySnapshot | null,
): void {
...
interface PluginCenterBackend {
  ...
  readonly readTransactionFingerprint: (
  ...
  if (!app.isPackaged) {
    ...
    return {
      ...
      shippedBundleManifests: [
        ...
        join(
          REPOSITORY_ROOT,
          "packages/examples/ff-llm-wiki-plugin/package.json",
        ),
      ],
...
function assertHostArtifacts(paths: ReturnType<typeof hostPaths>): void {
  ...
  if (!existsSync(paths.packageManagerEntry)) {
    throw new Error(
      `desktop package-manager entry is missing: ${paths.packageManagerEntry}`,
    );
  }
...
async function holdCurrentWindowFrame(
  window: BrowserWindow,
): Promise<HeldReloadFrame | undefined> {
  ...
  try {
    await held.webContents.loadURL(
...
async function reloadWindowHost(
  window: BrowserWindow,
  origin: string,
  primaryPage?: string,
): Promise<void> {
...
async function createMainWindow(): Promise<BrowserWindow> {
  ...
  window.webContents.on("will-navigate", (event, url) => {
    ...
    if (
      isRecoveryPageUrl(url) ||
      (currentOrigin !== undefined && hasOrigin(url, currentOrigin))
    )
      return;
...
function registerDesktopBridge(): PluginCenterBackend {
  ...
  const systemComponents = deriveProtectedSystemComponents(
    paths.shippedBundleManifests,
  );
  const readFingerprint = (
    ...
    readProfileCompatibilityFingerprint({
      homeDirectory: resolveDshHome(),
      profileName: "web",
  ...
  const compatibility = new PluginCompatibilityService(catalog, (selection) =>
    readFingerprint(selection, pluginOperationController?.active ?? false),
  );
  ...
  ipcMain.handle(DESKTOP_CHANNELS.updatesGet, () =>
    updateController?.getState(),
  );
  ipcMain.handle(DESKTOP_CHANNELS.updatesCheck, () =>
    updateController?.check(),
  );
  ipcMain.handle(DESKTOP_CHANNELS.updatesDownload, () =>
    updateController?.download(),
  );
  ...
  const assertRecoverySender = (event: Electron.IpcMainInvokeEvent): void => {
    ...
    if (
      ...
      (!isRecoveryPageUrl(url) &&
        (origin === undefined || !hasOrigin(url, origin)))
    ) {
      throw new Error(
        "plugin recovery request did not originate from the owned Desktop window",
      );
    }
  };
  ...
  ipcMain.handle(
    DESKTOP_CHANNELS.catalogRefresh,
  ...
  ipcMain.handle(
    DESKTOP_CHANNELS.catalogCheckCompatibility,
    (event, value: unknown) => {
      assertDesktopSender(event);
  ...
  ipcMain.handle(
    DESKTOP_CHANNELS.presetSquareDetail,
    (event, value: unknown) => {
      assertDesktopSender(event);
  ...
  ipcMain.handle(
    DESKTOP_CHANNELS.presetSquarePreviewInstall,
    (event, value: unknown) => {
      assertDesktopSender(event);
  ...
  ipcMain.handle(
    DESKTOP_CHANNELS.presetSquareInstall,
    (event, value: unknown) => {
      assertDesktopSender(event);
  ...
  ipcMain.handle(
    DESKTOP_CHANNELS.presetSquareRuntimeCheck,
    (event, value: unknown) => {
      assertDesktopSender(event);
  ...
  ipcMain.handle(
    DESKTOP_CHANNELS.presetSquareRuntimeInstall,
    (event, value: unknown) => {
      assertDesktopSender(event);
      const request = decodePresetRuntimeRequest(value);
      if (presetRuntimeController === undefined)
  ...
  ipcMain.handle(DESKTOP_CHANNELS.installedPluginsList, async (event) => {
    ...
    const runtimeEvidence =
      generation === undefined
        ? null
        : await new PluginRuntimeVerifier()
            .readEvidence(generation.origin)
            .catch(() => null);
  ...
  ipcMain.handle(
    DESKTOP_CHANNELS.pluginOperationStart,
    async (event, value: unknown) => {
      assertDesktopSender(event);
  ...
  ipcMain.handle(
    DESKTOP_CHANNELS.pluginOwnedDataRemove,
    async (event, value: unknown) => {
      assertDesktopSender(event);
      const remover = pluginOwnedDataRemover;
      if (remover === undefined)
  ...
  ipcMain.handle(
    DESKTOP_CHANNELS.pluginOwnedDataRetain,
    async (event, value: unknown) => {
      assertDesktopSender(event);
      const remover = pluginOwnedDataRemover;
      if (remover === undefined)
  ...
  ipcMain.handle(
    DESKTOP_CHANNELS.pluginRecoveryRetry,
    async (event, value: unknown) => {
      assertRecoverySender(event);
  ...
  ipcMain.handle(
    DESKTOP_CHANNELS.pluginRecoveryExport,
    async (event, value: unknown) => {
      assertRecoverySender(event);
  ...
  return {
    ...
    readTransactionFingerprint: (selection) =>
      readFingerprint(selection, false),
    systemComponents,
    paths,
  };
}
...
async function initializePluginOperations(
  backend: PluginCenterBackend,
): Promise<PluginStartupRecoveryResult> {
  const currentHost = host;
  const currentLifecycle = lifecycle;
  if (currentHost === undefined || currentLifecycle === undefined) {
    throw new Error(
      "plugin operation backend requires the current Host and window lifecycle",
    );
  }
  ...
  const snapshotStore = new ProfileSnapshotStore(
    profileDirectory,
    join(root, "snapshots"),
  );
  const ownedDataAuthorityStore = new PluginOwnedDataAuthorityStore(
    join(root, "owned-data-authority"),
  );
  ...
  const recovery = new PluginRecoveryController({
    ...
    runtimeVerifier,
    reloadHost: (origin) =>
      currentLifecycle.reloadHost(origin, PLUGIN_CENTER_PAGE_ID),
  });
  ...
  recovery.subscribe((snapshot) => {
    ...
    for (const window of BrowserWindow.getAllWindows()) {
      ...
      if (
        snapshot.phase === "recovery-failed" &&
        !pluginRecoverySafeMode &&
  ...
  const startup = await preparePluginCenterStartup({
    ...
    startNormalHost: async () => {
      ...
      initProfile(profileDirectory, [
        ...webProfileBundles,
        ...BUILT_IN_APPLICATION_BUNDLES,
      ]);
      reconcileBuiltInApplications(
        profileDirectory,
        BUILT_IN_APPLICATION_BUNDLES,
      );
      for (const manifestPath of backend.paths.shippedBundleManifests) {
        ...
        if (
          ...
          BUILT_IN_APPLICATION_BUNDLES.includes(
      ...
      if (dshmarketMigration.removedEntries > 0) {
        console.warn(
          `removed ${dshmarketMigration.removedEntries} legacy dshmarket registration(s) before Host start`,
        );
      }
      ...
      for (const item of compatibility.deactivated) {
        console.warn(
          `disabled incompatible plugin before Host start: ${item.pluginId}@${item.version}`,
        );
      }
      if (
        await migrateLegacyBundledContentPreset({
          homeDirectory: dshHome,
          bundledPresetRoot: bundledPresetRoot(),
        })
      ) {
        console.warn(
          "migrated legacy bundled content Preset before Host start",
        );
      }
      return await currentHost.start();
    },
    startSafeHost: async () =>
      currentHost.current ?? (await currentHost.start()),
  });
  if (
    startup.recovery?.operationId !== UNREADABLE_PLUGIN_JOURNAL_OPERATION_ID
  ) {
      ...
      for (const window of BrowserWindow.getAllWindows()) {
        if (!window.isDestroyed())
          window.webContents.send(
            DESKTOP_CHANNELS.pluginOperationState,
            operation,
          );
      }
    });
