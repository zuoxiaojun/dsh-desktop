...
description: EXTEND.md YAML schema for ai-content-images user preferences
---
...
generation_batch_size: 1       # 本案例固定串行
...
| `preferred_image_backend` | string | `dsh-content-imagegen` | 本案例场景 DSH Plugin；内部受限调用本机 `codex exec --ephemeral`。 |
| `generation_batch_size` | int | 1 | 本案例固定串行，保护本地 Codex 会话。 |
