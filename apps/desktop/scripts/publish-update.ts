...
export interface ReleaseObjectStore {
  head(
    objectKey: string,
  ): Promise<{ readonly size: number; readonly sha512?: string } | undefined>;
 ...
 */
export function collectReleaseBundle(
  directories: readonly string[],
  options: CollectOptions,
): ReleaseBundle {
  if (directories.length === 0)
    throw new Error("至少需要一个 --dir 发布目录。");
  ...
  for (const input of directories) {
    ...
    if (!statSync(directory).isDirectory())
      throw new Error(`发布目录不是文件夹：${directory}`);
    const metadataNames = readdirSync(directory).filter(
      (name) => name.endsWith(".yml") && !name.startsWith("builder-"),
    );
    if (metadataNames.length === 0)
      throw new Error(`发布目录缺少渠道 yml：${directory}`);

    for (const metadataName of metadataNames) {
      if (metadataName.endsWith("-linux.yml")) {
        throw new Error(`Linux 在线更新发布尚未接入：${metadataName}`);
      }
      const platform: Platform = metadataName.endsWith("-mac.yml")
        ? "mac"
        : "windows";
      ...
      const hasMacZip = metadata.files.some(
        (file) => extname(file.url).toLowerCase() === ".zip",
      );
      if (platform === "mac" && !hasMacZip) {
        if (
          !options.allowCurrentBaseline ||
          metadata.version !== options.currentVersion
        ) {
      ...
      for (const file of metadata.files) {
        ...
        addPayload(
          payloads,
          payloadFor("artifact", artifactPath, artifactName),
        );

        const blockmapName = `${artifactName}.blockmap`;
        const blockmapPath = join(directory, blockmapName);
        if (!existsSync(blockmapPath))
          throw new Error(`更新载荷缺少 blockmap：${blockmapPath}`);
        addPayload(
          payloads,
          payloadFor("blockmap", blockmapPath, blockmapName),
        );
      }
  ...
  if (versions.size !== 1)
    throw new Error(`发布目录版本不一致：${[...versions].join(", ")}`);
  if (channels.size !== 1)
    throw new Error(`发布目录渠道不一致：${[...channels].join(", ")}`);
  const version = onlyValue(versions, "版本");
  if (version !== options.currentVersion) {
    throw new Error(
      `发布版本 ${version} 与 Desktop package.json ${options.currentVersion} 不一致。`,
    );
  }
  return {
    version,
    channel: onlyValue(channels, "渠道"),
...
export async function publishReleaseBundle(
  ...
  const immutable = bundle.payloads.filter(
    (payload) => payload.kind !== "metadata",
  );
  const metadata = bundle.payloads.filter(
    (payload) => payload.kind === "metadata",
  );
  for (const payload of [...immutable, ...metadata]) {
    ...
    if (payload.kind !== "metadata" && existing !== undefined) {
      if (
        ...
        existing.sha512 !== payload.sha512
      ) {
        throw new Error(
          `版本化对象已存在但内容不同，请提升版本号：${payload.objectKey}`,
        );
      }
...
class AliyunReleaseStore implements ReleaseObjectStore {
  ...
  async put(payload: ReleasePayload): Promise<void> {
    ...
    if (result.res.status !== 200)
      throw new Error(
        `OSS PutObject 返回 ${String(result.res.status)}：${payload.objectKey}`,
      );
  }
}
...
function validateReleaseFile(path: string, expected: ReleaseFile): void {
  if (!existsSync(path) || !statSync(path).isFile())
    throw new Error(`渠道元数据引用的文件不存在：${path}`);
  const size = statSync(path).size;
  if (size !== expected.size)
    throw new Error(`文件大小与渠道元数据不一致：${path}`);
  const sha512 = digest(path);
  if (sha512 !== expected.sha512)
    throw new Error(`文件 SHA-512 与渠道元数据不一致：${path}`);
}
...
function addPayload(
  payloads: Map<string, ReleasePayload>,
...
async function verifyPublicMetadata(payload: ReleasePayload): Promise<void> {
  ...
  if (!response.ok)
    throw new Error(
      `公网渠道清单不可读：${url.origin}${url.pathname} (${String(response.status)})`,
    );
  const actual = Buffer.from(await response.arrayBuffer());
  const expected = readFileSync(payload.localPath);
  if (!actual.equals(expected))
    throw new Error(`公网渠道清单与本地不一致：${url.origin}${url.pathname}`);
...
async function main(): Promise<void> {
  ...
  console.log(
    `发布校验通过：channel=${bundle.channel} version=${bundle.version} files=${String(bundle.payloads.length)}`,
  );
  ...
  if (accessKeyId === undefined || accessKeySecret === undefined) {
    throw new Error(
      "缺少 ALIYUN_OSS_ACCESS_KEY_ID 或 ALIYUN_OSS_ACCESS_KEY_SECRET。",
    );
  }
