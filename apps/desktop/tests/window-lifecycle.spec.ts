...
describe("desktop window lifecycle", () => {
  it("recognizes only the exact private installer quit argument", () => {
    expect(
      isInstallerQuitRequest(["DSH Desktop.exe", INSTALLER_QUIT_ARGUMENT]),
    ).toBe(true);
    expect(
      isInstallerQuitRequest([
        "DSH Desktop.exe",
        `${INSTALLER_QUIT_ARGUMENT}=true`,
      ]),
    ).toBe(false);
  ...
  it("reloads the existing window for the current host generation without changing visibility", async () => {
    const window = fakeWindow({ visible: false });
    const loadCurrentHost = vi
      .fn<(_window: DesktopWindow, origin: string) => Promise<void>>()
    ...
    expect(loadCurrentHost).toHaveBeenCalledWith(
      window,
      "http://127.0.0.1:5123",
      "plugin-center",
    );
