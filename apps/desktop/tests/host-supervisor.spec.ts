...
describe("desktop Host readiness", () => {
  it("extracts the canonical URL from arbitrarily chunked output and ignores unrelated URLs", () => {
    ...
    expect(
      parser.push("Node warning: see https://nodejs.org/docs\n"),
    ).toBeUndefined();
    expect(parser.push("dsh we")).toBeUndefined();
    expect(parser.push("b: http://127.0.")).toBeUndefined();
    expect(
      parser.push("0.1:4173 (LAN: http://192.0.2.10:4173)"),
    ).toBeUndefined();
  ...
  it("accepts a complete unterminated readiness line when the stream ends", () => {
    ...
    expect(
      parser.push("diagnostic\ndsh web: http://localhost:51234"),
    ).toBeUndefined();
...
describe("desktop Host supervisor", () => {
  ...
  it("reports output when the Host exits before readiness", async () => {
    ...
    await expect(starting).rejects.toThrow(
      /exited before readiness \(code 7, signal null\).*configuration rejected/su,
    );
  });
  ...
  it("forbids starting after shutdown", async () => {
    ...
    await expect(supervisor.start()).rejects.toThrow(
      "desktop Host cannot start after shutdown",
    );
    await expect(supervisor.restart("late request")).rejects.toThrow(
      "desktop Host cannot restart after shutdown",
    );
  ...
  it("times out startup once and terminates the unready child", async () => {
    ...
    const rejected = expect(starting).rejects.toThrow(
      "desktop Host readiness timed out after 25ms",
    );
  ...
  it("serializes queued restarts so each operation owns one stop and replacement", async () => {
    ...
    await expect(firstRestart).resolves.toEqual({
      id: 2,
    ...
    await expect(secondRestart).resolves.toEqual({
      id: 3,
...
describe("desktop Host process", () => {
  it("opts the packaged Electron executable into its Node runtime", async () => {
    ...
    spawnDshWeb({
      ...
      cliEntry:
        "/Applications/DSH Desktop.app/Contents/Resources/host/node_modules/@deepseek-ai/dsh/lib/bin.js",
      cwd: "/Users/tester",
      env: { DSH_DESKTOP: "1" },
    ...
    expect(spawn).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        env: { DSH_DESKTOP: "1", ELECTRON_RUN_AS_NODE: "1" },
      }),
    );
  });
});
