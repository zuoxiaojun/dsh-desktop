...
Bundle names resolve from the dsh installation first, then from the profile directory. In-box bundles (`@deepseek-ai/dsh-base`, `@deepseek-ai/dsh-web-app`, `@deepseek-ai/dsh-headless`) therefore always come from the same installation as the running `dsh`; out-of-tree bundles come from the profile's pnpm-managed `node_modules`. A bare plugin `name` in any patch row follows the same order: the installation copy wins when present, and a package absent from the installation falls back through the profile directory's Node resolution. This prevents a third-party dependency from shadowing the running Harness core while preserving profile-only plugins. `healProfilesModuleFallback` still maintains `$DSH_HOME/profiles/node_modules` for callers that boot without an explicit installation anchor.
