...
import { isDshNpmReleaseAppDirectory } from './release/families.ts'
...
/** Studio-owned packages released independently from the upstream dsh npm family. */
const studioIndependentPackageDirectories = new Set([
  'packages/examples/ff-llm-wiki-plugin',
  'packages/examples/plugin-center-fixture',
  'packages/examples/plugin-center-skill-fixture',
])
/** Directories whose packages this repository publishes: one release member each. */
const releaseMemberDirectory = /^(?:packages\/(?!experimental\/)[^/]+\/[^/]+|vendor\/[^/]+)$/
...
function checkWorkspace({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  } else if (studioIndependentPackageDirectories.has(dir)) {
    const fixture = dir.endsWith('plugin-center-fixture') || dir.endsWith('plugin-center-skill-fixture')
    if (fixture && manifest.private !== true) errors.push(`${label}: Desktop fixture package must stay private`)
    if (!fixture && manifest.private === true) errors.push(`${label}: independently published Studio package must not be private`)
  } else if (releaseMemberDirectory.test(dir) || isDshNpmReleaseAppDirectory(dir)) {
  ...
  if (isDshNpmReleaseAppDirectory(dir) && manifest.name?.startsWith('@deepseek-ai/')) {
  ...
  if (dir.startsWith('packages/') && manifest.name?.startsWith('@deepseek-ai/dsh-')) {
    ...
    if (!studioIndependentPackageDirectories.has(dir) && manifest.version !== repositoryVersion) {
