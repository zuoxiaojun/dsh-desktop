...
const scriptPath = resolve(process.argv[2] ?? 'scripts/record/scripts/feat-pptfast-demo.json')
...
const previewUrl = pathToFileURL(join(outDir, 'pptfast-demo/preview/preview.html')).href
...
const installedPptfast = page.getByRole('button', { name: '管理已安装插件：@liustack/pptfast' })
if (await installedPptfast.count()) {
  await installedPptfast.click()
  const row = page.locator('li[data-installed-plugin^="npm.liustack.pptfast."]')
...
try {
  ...
  await search.pressSequentially('pptfast', { delay: 45 })
  mark('搜索 pptfast')
  const card = page.locator('li').filter({ has: page.getByRole('button', { name: '查看详情：@liustack/pptfast' }) })
  ...
  for (const index of [1, 2, 3, 4]) {
    const thumb = page.locator(`#pf-thumb-${String(index)}`)
    await click(thumb, `查看第 ${String(index + 1)} 页`, 45)
    await page.waitForTimeout(650)
  }
  mark('完成：可编辑 PPTX 已生成')
...
if (page.url() === previewUrl) await page.goto(returnUrl, { waitUntil: 'domcontentloaded' }).catch(() => {})
await frameQueue
