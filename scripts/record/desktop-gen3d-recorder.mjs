...
const scriptPath = resolve(process.argv[2] ?? 'scripts/record/scripts/feat-gen3d-demo.json')
...
const previewUrl = pathToFileURL(join(outDir, 'gen3d-demo/viewer.html')).href
...
try {
  ...
  await search.pressSequentially('dsh-gen3d', { delay: 42 })
  mark('搜索 dsh-gen3d')
  const card = page.locator('li').filter({ hasText: 'dsh-gen3d' }).first()
  ...
  await page.mouse.move(1380, 400, { steps: 45 })
  mark('旋转查看角色')
  await page.waitForTimeout(1900)
  await page.mouse.move(620, 650, { steps: 55 })
  await page.waitForTimeout(1900)
  await page.mouse.move(1120, 520, { steps: 40 })
  mark('完成：3D 资产已生成')
  await page.waitForTimeout(1200)
} catch (error) {
