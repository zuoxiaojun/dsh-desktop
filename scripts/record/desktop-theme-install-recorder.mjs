...
const scriptPath = resolve(process.argv[2] ?? 'scripts/record/scripts/feat-theme-install.json')
...
let recordingError
try {
  mark('开始：151 款皮肤模板未安装')
  await page.waitForTimeout(300)
  await moveTo(search)
  await page.mouse.click(cursorX, cursorY)
  await search.pressSequentially('@dsh-suite/themes', { delay: 38 })
  mark('搜索 @dsh-suite/themes')
  const card = page.locator('li').filter({ hasText: '@dsh-suite/themes' }).first()
