...
const scriptPath = resolve(process.argv[2] ?? 'scripts/record/scripts/feat-pet-lifecycle.json')
...
try {
  ...
  await search.pressSequentially('mypet', { delay: 45 })
  mark('搜索 mypet')
  const card = page.locator('li').filter({ has: page.getByRole('button', { name: '查看详情：mypet' }) })
  await card.waitFor({ timeout: 30_000 })
  await page.waitForTimeout(300)
  await click(card.getByRole('button', { name: '安装', exact: true }), '点击安装')
  await confirmDialog('确认安装')
  await waitOperation({ petVisible: true })
  await completeOnboarding('首次领养')
  const panel = page.locator('.dshpet-panel')
  await click(panel.locator('button.dshpet-btn').filter({ hasText: '抚摸' }).first(), '抚摸 Pet')
  await page.waitForTimeout(600)
  await click(panel.getByRole('button', { name: '关闭' }), '关闭 Pet 面板')

  await click(page.getByRole('button', { name: '管理已安装插件：mypet' }), '打开 Pet 管理')
  let row = page.locator('li[data-installed-plugin^="npm.mypet."]')
  await click(row.locator('[data-action="disable"]'), '选择停用')
  await confirmDialog('停用')
  await waitOperation({ petVisible: false })

  await click(page.getByRole('button', { name: '管理已安装插件：mypet' }), '打开已停用 Pet')
  row = page.locator('li[data-installed-plugin^="npm.mypet."]')
  await click(row.locator('[data-action="enable"]'), '选择启用')
  await confirmDialog('启用')
  await waitOperation({ petVisible: true })
  await completeOnboarding('重新启用')
  mark('完成：Pet 已启用并运行')
