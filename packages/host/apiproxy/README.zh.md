...
仅环回可用的二进制 `POST /api/agent-preset.import` 用于预览或安装一个经 Desktop 校验的 `.dshpreset` 归档。导入器限制压缩、展开、单文件和文件数量大小，拒绝不安全路径与意外顶层文件，要求带版本的 manifest 和 `agent.cordis.yml`，用普通 Preset loader 扫描候选项，拒绝覆盖，并通过一次目录原子重命名提交合法用户 Preset。预览使用同一套校验，但不写入目标目录。
...
`vision.*` 领域把图片提供方配置保留在 Host。除固定的百炼与 OpenRouter 路由外，`vision.enable` 还接受 Ollama、vLLM、SGLang，或通用 OpenAI-compatible API Base，并要求精确模型 ID，API Key 可选。Host 校验 HTTP(S) URL 结构，追加 `/chat/completions`，拒绝重定向，继续执行既有的 10 MB 图片、1 MB 响应与 60 秒时限，并只在提供方返回可用视觉描述后保存 endpoint。选择自托管路由时绝不会回退到任一云端地址。
