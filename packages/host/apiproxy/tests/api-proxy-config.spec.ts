...
import { VISION_SETTINGS_NAMESPACE } from '../src/vision-enhancement.ts'
...
describe('settings domain', () => {
  ...
  it('refuses direct vision enable writes while preserving disable and the atomic Host gate', async () => {
    ...
    const bypass = expectErr(await api.settings.update(request({ ns: 'vision-enhancement', patch: { enabled: true } })))
    expect(bypass.code).toBe('settings-rejected')
    expect(bypass.message).toContain('验证并开启')
    expect(expectOk(await api.settings.update(request({ ns: 'vision-enhancement', patch: { enabled: false } }))).value)
      .toEqual({ enabled: false })

    const result = expectOk(await api.vision.enable(request({
      mediaType: 'image/webp', data: 'aW1hZ2U=', apiKey: 'bailian-test-key', name: 'cat.webp',
    })))
    expect(result.description).toBe('一只小猫。')
    expect(enable).toHaveBeenCalledWith(expect.objectContaining({ apiKey: 'bailian-test-key' }), undefined)
  })
})
