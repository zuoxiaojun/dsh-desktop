...
import { VISION_SETTINGS_NAMESPACE, type VisionEnhancementRuntime } from './vision-enhancement.ts'
...
export interface ApiProxyDefaults {
   ...
   */
  canOpenPath?: () => boolean
  /** Desktop visual augmentation runtime; absent preserves the upstream text/image gates. */
  ...
  async function settingsWrite(
    ...
    if (branded === VISION_SETTINGS_NAMESPACE) {
      const isDisableOnly = mode === 'update'
        ...
        && (section as { enabled?: unknown }).enabled === false
      if (!isDisableOnly) {
        return rejected(new Error('视觉能力只能通过“验证并开启”启用；普通设置接口只允许关闭。'))
      }
    }
    try {
