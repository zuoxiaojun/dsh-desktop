...
async function bench() {
  ...
  const closePrimaryPage = vi.fn()
  ctx.provide('workspaces', {
    create, startSession, rename, insertSessionBefore,
  } as never)
  ctx.provide('sessions', { open, clear, search, searchResultLimit: 20, binding, fork } as never)
  ctx.provide('layout', { closePrimaryPage } as never)
  ...
  return {
    ...
    insertSessionBefore, open, clear, search, renameSession, binding, fork, closePrimaryPage,
  }
}
...
describe('ui-workspace apply', () => {
  ...
  it('routes browser actions and picker creation to the services', async () => {
    ...
    expect(b.closePrimaryPage).toHaveBeenCalledTimes(3)
    ...
    expect(b.closePrimaryPage).toHaveBeenCalledTimes(4)
