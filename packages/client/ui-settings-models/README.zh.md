...
**添加本地模型**是 Ollama、vLLM、SGLang 与通用 OpenAI-compatible 服务的直接入口。选择框架后会预填约定的本机 `/v1` 地址和稳定的提供方 id，地址仍可编辑，API Key 明确为可选；后续复用自定义提供方流程已有的模型读取／手填与 profile 写入。每个预设都会记录 `allowUnauthenticated`，因此密钥留空时，适配器会使用只存在于运行期的非机密 SDK 占位值，而不会存储假凭据；后续填写的真实密钥具有更高优先级。引导卡不显示由预设确定的 Provider ID 和协议；已经配置的预设会被禁用，并继续通过对应的提供方行管理。Studio 不会启动推理服务或下载模型权重。
