...
Native directory-picker surface: the browser half of the native picking interaction. It fills ui-workspace's two directory-flow holes (`conversation.hero.workspace.directoryFlow` and `sidebar.workspaces.directoryFlow`) with a renderless occupant that answers each `open` request through Electron's fixed preload picker when present, otherwise through the local Host's `ctx.workspaces.pickDirectory()`, then reports exactly one outcome — a picked path, a cancellation, or a failure — back through the owner conversation. The Host fallback belongs to [`dsh-host-directory-picker-native`](../../host/directory-picker-native/README.md); mounting this package composes both halves from one cordis.yml row, while packaged Electron keeps its operating-system dialog in the main process and no client code branches on a capability kind.
...
- **No cancellation of an open Host chooser** — the Host wire has no per-request abort, so a chooser already on the host display cannot be closed from the browser; a discarded settlement is simply ignored. Electron owns its dialog lifetime in the main process.
