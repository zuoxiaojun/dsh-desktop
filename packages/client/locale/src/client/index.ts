...
/**
 * Desktop starts in the product fallback language while ordinary Web follows
 * the browser. An explicit Host preference may replace either provisional
 * value after plugin activation.
 */
function resolveInitialLocale(): LocaleId {
  if (isDesktopRenderer()) return 'zh'
  return detectBrowserLocale() ?? FALLBACK_LOCALE
}

function isDesktopRenderer(): boolean {
  return typeof document !== 'undefined' && document.documentElement.dataset.dshDesktop === 'true'
}
