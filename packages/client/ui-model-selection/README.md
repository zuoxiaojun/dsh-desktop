...
Model selection plugin, browser half: TWO entries over ONE per-session directory owned by `ModelDirectoryResolver` (`ctx.modelDirectories`). For ordinary sessions, the `/model` popupSelect contribution (registered through `ctx.commandUi`) and the composer's named `conversation.input.model` seat both load the session's advisory directory through `session.models` and submit through `session.selectModel` via the same `ModelDirectory` instance. The compact composer trigger opens a two-level Model/reasoning menu: its primary rows stay visible while hover, keyboard focus, or click opens the matching selection card on their right. The visual gutter between both cards is pointer-safe and a short leave grace period keeps the side card stable while the pointer crosses it; click and keyboard focus continue to open the same deterministic card. Models stay provider-grouped, while the selected exact model supplies its adapter-owned effort ids, descriptions, and default. DeepSeek's `off`, `low`, `high`, and `max` ids render as localized Thinking off, Low thinking, Deep thinking, and Maximum thinking choices under a Thinking mode row; other providers retain their adapter-owned names. `/model` applies the selected model's default effort, and the composer can then choose any advertised effort.
...
- **No arbitrary effort input** — the composer offers only the exact model's adapter-advertised levels; localization changes labels, never ids, and an adapter without reasoning metadata leaves the reasoning row absent.
