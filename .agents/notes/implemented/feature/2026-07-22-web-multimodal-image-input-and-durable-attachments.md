...
Model selection never rejects solely because durable history or a pending inbox item contains an image. A text-only selection leaves those messages intact; request projection replaces images the selected model cannot receive with deterministic omission text, while a later image-capable selection can consume the original durable references. New image prompt admission still evaluates the model selected at that serialized commit point.
