...
循环通过进程全局的 `Symbol.for` 键访问 ToolRuntime 调度器。该键仍属于私有实现，也不会出现在生成的具名服务 API 中；但即使插件加载器通过多个模块实例求值 `@deepseek-ai/dsh-tools`，它的身份仍保持一致。若使用普通的模块局部 `Symbol`，打包运行时一旦出现该情形，所有工具调用都会在进入 `prepare` 前失败。
