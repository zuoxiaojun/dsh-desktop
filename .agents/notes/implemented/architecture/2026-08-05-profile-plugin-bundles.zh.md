...
解析在构造上就是双锚点的：`dsh.profile.bundles` 中的名称和 patch 行中的裸插件名称都先从 dsh 安装目录解析，再从 profile 目录解析。因此，即使某个树外插件依赖另一版 Harness，内置组合包和核心插件仍始终来自与运行中 `dsh` 相同的安装；安装中不存在的包才回退到 profile 里由 pnpm 管理的 `node_modules`。对于未显式提供安装锚点的调用方，受维护的扁平回退目录 `$DSH_HOME/profiles/node_modules` 仍然可用。
