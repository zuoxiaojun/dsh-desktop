...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/desktop` first reuses the complete Web application through a supervised loopback child; that replaceable shell preserves the path for the same web client packages to move to an IPC fetch carrier ([decision](2026-08-14-electron-loopback-web-supervisor.md)).
...
| Carrier layer | `dsh-host-webserver` | Web HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Web (browser access) only; zero workspace dependencies (the registry arrives by structural injection); the Studio desktop shell supervises an ordinary Web-profile child and never imports the carrier into Electron |
...
The applications preserve the division: the Web application mounts Host, carrier, and browser composition; the first desktop application supervises that complete Web application without changing its composition; and `dsh --profile headless` mounts a direct core runner with zero Host, HTTP, or ports. ACP-class protocol bridges do not follow the client-carrier checklist: they expose core to the external ecosystem and mount directly via `ctx.plugin(entry-point plugin)` without fetch.
...
| IPC bridge subclass (reserved; the [desktop shell](2026-08-14-electron-loopback-web-supervisor.md) currently supervises the Web carrier) | an Electron shell | IPC serialization round trip | swaps only doFetch; contract and base class stay unchanged |
