...
Resolution is two-anchored by construction: `dsh.profile.bundles` names and bare plugin names in patch rows resolve from the dsh installation first, then the profile directory. In-box bundles and core plugins therefore always come from the same installation as the running `dsh`, even when an out-of-tree plugin depends on another Harness version; a package absent from the installation falls back to the profile's pnpm-managed `node_modules`. The maintained flat fallback `$DSH_HOME/profiles/node_modules` remains available to callers that boot without an explicit installation anchor.
